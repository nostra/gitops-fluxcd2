...
.footnote[xx.12.2023 Erlend Nossum]
???
...
kubectl -n default apply -f sshd.yaml
popd
```
Test the connection:
```bash
...
- Alternative: `GIT_SSH_COMMAND="ssh -i ~/.ssh/fluxpres -p 30022" git clone ssh://fluxpres@localhost:30022/home/fluxpres/flux-system.git`
---
## ...  and enable GitOps with kustomize
.left-column[
## system
...
```bash
kubectl port-forward -n backend svc/nginx-test 8080
```
* http://localhost:8080
]
???
